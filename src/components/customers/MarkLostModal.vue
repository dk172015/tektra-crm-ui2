<template>
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4">
    <div class="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
      <div class="mb-4">
        <h3 class="text-lg font-semibold text-slate-900">Cập nhật mất khách</h3>
        <p class="mt-1 text-sm text-slate-500">
          Nhập lý do mất khách trước khi chuyển sang tab Mất khách.
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div class="md:col-span-2">
          ...
          <input v-model="form.reason" class="crm-input" placeholder="Ví dụ: Giá cao, khách đổi khu vực..." />
        </div>

        <div>
          <label class="crm-label">Đối thủ / bên chốt</label>
          <input v-model="form.competitor_name" class="crm-input" placeholder="Tên đơn vị khác" />
        </div>

        <div>
          <label class="crm-label">Giá bên khác</label>
          <input v-model="form.lost_price" type="number" min="0" class="crm-input" placeholder="Nhập giá nếu biết" />
        </div>
        ...
        <div class="md:col-span-2">
          ...
          <textarea v-model="form.note" rows="4" class="crm-input h-auto py-3" placeholder="Ghi chú thêm..." />
        </div>
      </div>
      ...
      <div class="mt-6 flex justify-end gap-3">
        <button class="crm-btn crm-btn-secondary" @click="$emit('update:modelValue', false)">Hủy</button>
        <button class="crm-btn crm-btn-primary" :disabled="loading" @click="submit">
          {{ loading ? 'Đang lưu...' : 'Xác nhận mất khách' }}
        </button>
      </div>
    </div>
  </div>
...
import { markLostApi } from '../../api/customers'
...
const submit = async () => {
  ...
  loading.value = true
  try {
    await markLostApi(props.customerId, {
      reason: form.reason,
