<script setup>
import { onMounted, reactive, ref } from 'vue'
import { useRouter } from 'vue-router'
import MainLayout from '../../layouts/MainLayout.vue'
import { useAuthStore } from '../../stores/auth'
import {
  getRevenueSummaryApi,
  getRevenueByPeriodApi,
  getRevenueBySaleApi,
} from '../../api/reports'

const router = useRouter()
const auth = useAuthStore()

const loading = ref(false)

const summary = ref({
  today_revenue: 0,
  month_revenue: 0,
  year_revenue: 0,
  total_revenue: 0,
})

const periodRows = ref([])
const saleRows = ref([])

const filters = reactive({
  type: 'month',
  date_from: '',
  date_to: '',
})

const formatMoney = (value) => {
  const number = Number(value || 0)
  return new Intl.NumberFormat('vi-VN').format(number)
}

const formatCompactMoney = (value) => {
  const number = Number(value || 0)
  if (number >= 1000000000) return `${(number / 1000000000).toFixed(1)}B`
  if (number >= 1000000) return `${(number / 1000000).toFixed(1)}M`
  if (number >= 1000) return `${(number / 1000).toFixed(1)}K`
  return `${number}`
}

const saleMax = () => {
  return Math.max(...saleRows.value.map((item) => Number(item.total_revenue || 0)), 0)
}

const barWidth = (value, max) => {
  const numeric = Number(value || 0)
  const maxValue = Number(max || 0)
  if (!maxValue || !numeric) return '0%'
  return `${Math.max(8, (numeric / maxValue) * 100)}%`
}

const fetchData = async () => {
  loading.value = true
  try {
    const params = {
      type: filters.type,
      date_from: filters.date_from || undefined,
      date_to: filters.date_to || undefined,
    }

    const [summaryRes, periodRes, saleRes] = await Promise.all([
      getRevenueSummaryApi(params),
      getRevenueByPeriodApi(params),
      getRevenueBySaleApi({
        date_from: filters.date_from || undefined,
        date_to: filters.date_to || undefined,
      }),
    ])

    summary.value = summaryRes.data
    periodRows.value = periodRes.data.data || []
    saleRows.value = saleRes.data.data || []
  } catch (error) {
    alert(error?.response?.data?.message || 'Không tải được báo cáo doanh thu.')
  } finally {
    loading.value = false
  }
}

const goDeals = (extra = {}) => {
  router.push({
    path: '/customer-deals',
    query: {
      date_from: filters.date_from || undefined,
      date_to: filters.date_to || undefined,
      ...extra,
    },
  })
}

onMounted(fetchData)
</script>

<template>
  <MainLayout>
    <div class="space-y-5">
      <div class="flex items-start justify-between gap-4">
        <div>
          <div class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-500 shadow-sm">
            <span class="h-2 w-2 rounded-full bg-indigo-500"></span>
            Revenue Report
          </div>
          <h1 class="mt-3 text-3xl font-semibold tracking-tight text-slate-900">Báo cáo doanh thu</h1>
          <p class="mt-2 text-sm text-slate-500">
            {{ auth.isAdmin ? 'Xem tổng quan doanh thu toàn hệ thống.' : 'Chỉ xem doanh thu của bạn.' }}
          </p>
        </div>

        <div class="grid gap-2 rounded-3xl border border-slate-200 bg-white p-3 shadow-sm sm:grid-cols-[1fr_1fr_1fr_auto]">
          <select
            v-model="filters.type"
            class="h-11 rounded-2xl border border-slate-300 px-4 text-sm outline-none"
          >
            <option value="day">Theo ngày</option>
            <option value="month">Theo tháng</option>
            <option value="year">Theo năm</option>
          </select>

          <input
            v-model="filters.date_from"
            type="date"
            class="h-11 rounded-2xl border border-slate-300 px-4 text-sm outline-none"
          />
          <input
            v-model="filters.date_to"
            type="date"
            class="h-11 rounded-2xl border border-slate-300 px-4 text-sm outline-none"
          />

          <button
            class="h-11 rounded-2xl bg-slate-900 px-5 text-sm font-medium text-white hover:bg-slate-800"
            @click="fetchData"
          >
            Lọc
          </button>
        </div>
      </div>

      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <button
          class="rounded-3xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          @click="goDeals()"
        >
          <div class="text-xs uppercase tracking-wide text-slate-400">Doanh thu hôm nay</div>
          <div class="mt-2 text-3xl font-semibold text-slate-900">
            {{ formatCompactMoney(summary.today_revenue) }}
          </div>
        </button>

        <button
          class="rounded-3xl border border-emerald-200 bg-emerald-50 p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          @click="goDeals()"
        >
          <div class="text-xs uppercase tracking-wide text-emerald-600">Doanh thu tháng</div>
          <div class="mt-2 text-3xl font-semibold text-emerald-700">
            {{ formatCompactMoney(summary.month_revenue) }}
          </div>
        </button>

        <button
          class="rounded-3xl border border-indigo-200 bg-indigo-50 p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          @click="goDeals()"
        >
          <div class="text-xs uppercase tracking-wide text-indigo-600">Doanh thu năm</div>
          <div class="mt-2 text-3xl font-semibold text-indigo-700">
            {{ formatCompactMoney(summary.year_revenue) }}
          </div>
        </button>

        <button
          class="rounded-3xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          @click="goDeals()"
        >
          <div class="text-xs uppercase tracking-wide text-slate-400">Tổng doanh thu</div>
          <div class="mt-2 text-3xl font-semibold text-slate-900">
            {{ formatCompactMoney(summary.total_revenue) }}
          </div>
        </button>
      </div>

      <div class="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-base font-semibold text-slate-900">Doanh thu theo kỳ</h2>
              <p class="mt-1 text-xs text-slate-500">Tổng hợp theo bộ lọc đang chọn</p>
            </div>
            <button
              class="rounded-xl border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50"
              @click="router.push('/analytics/revenue')"
            >
              Mở phân tích
            </button>
          </div>

          <div v-if="loading" class="mt-5 space-y-3">
            <div v-for="i in 8" :key="i" class="h-11 animate-pulse rounded-2xl bg-slate-100"></div>
          </div>

          <div v-else class="mt-5 overflow-hidden rounded-2xl border border-slate-200">
            <table class="min-w-full text-sm">
              <thead class="bg-slate-50 text-left text-slate-500">
                <tr>
                  <th class="px-4 py-3 font-medium">Kỳ</th>
                  <th class="px-4 py-3 font-medium">Doanh thu</th>
                  <th class="px-4 py-3 font-medium text-right">Action</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  v-for="row in periodRows"
                  :key="row.label"
                  class="border-t border-slate-100 hover:bg-slate-50"
                >
                  <td class="px-4 py-3 font-medium text-slate-900">{{ row.label }}</td>
                  <td class="px-4 py-3">{{ formatMoney(row.revenue) }}</td>
                  <td class="px-4 py-3 text-right">
                    <button
                      class="rounded-xl border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50"
                      @click="goDeals()"
                    >
                      Xem deal
                    </button>
                  </td>
                </tr>
                <tr v-if="!periodRows.length">
                  <td colspan="3" class="px-4 py-10 text-center text-sm text-slate-500">
                    Không có dữ liệu.
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>

        <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-base font-semibold text-slate-900">Doanh thu theo sale</h2>
              <p class="mt-1 text-xs text-slate-500">Xếp hạng doanh thu trong kỳ</p>
            </div>
            <button
              class="rounded-xl border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50"
              @click="router.push('/analytics/revenue')"
            >
              Phân tích sâu
            </button>
          </div>

          <div v-if="loading" class="mt-5 space-y-3">
            <div v-for="i in 6" :key="i" class="h-12 animate-pulse rounded-2xl bg-slate-100"></div>
          </div>

          <div v-else class="mt-5 space-y-3">
            <button
              v-for="row in saleRows"
              :key="row.user_id"
              class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-left hover:bg-slate-50"
              @click="goDeals({ sale_id: row.user_id })"
            >
              <div class="flex items-center justify-between text-sm">
                <span class="font-medium text-slate-900">{{ row.user_name }}</span>
                <span class="text-slate-500">{{ formatCompactMoney(row.total_revenue) }}</span>
              </div>

              <div class="mt-2 h-2 rounded-full bg-slate-100">
                <div
                  class="h-full rounded-full bg-emerald-600"
                  :style="{ width: barWidth(row.total_revenue, saleMax()) }"
                ></div>
              </div>

              <div class="mt-2 text-xs text-slate-400">{{ row.total_deals }} hợp đồng</div>
            </button>

            <div v-if="!saleRows.length" class="rounded-2xl bg-slate-50 px-4 py-10 text-sm text-slate-500">
              Không có dữ liệu.
            </div>
          </div>
        </div>
      </div>
    </div>
  </MainLayout>
</template>